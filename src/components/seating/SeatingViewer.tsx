...
const SEAT_TAP_MIN_R_MOUSE = 5;
const SEAT_TAP_MIN_R_TOUCH = 9;
// Target seat screen radius (px) the next progressive zoom step aims for.
const SEAT_TAP_TARGET_R_MOUSE = 7;
const SEAT_TAP_TARGET_R_TOUCH = 11;
			...
			if (tappedSection && scale.current < MAX_SCALE * 0.95) {
				...
				if (seatScreenR < minTapR) {
					...
					if (!atSectionLevel) {
						...
						const stepScale = Math.min(MAX_SCALE, Math.max(scale.current * 1.01, targetScale));
