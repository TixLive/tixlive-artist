...
export interface SmShape {
	id: string;
	kind: 'rect' | 'label' | 'stairs' | 'arrow' | 'poly';
	x: number;
	y: number;
	w: number;
	h: number;
	...
	// 'poly': closed outline points, relative to (x, y); w/h = bounding box.
	// Generated by «Formă în jurul selecției» (smart hull around seats).
	pts?: [number, number][];
