...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	try {
		...
		out.eventsOk = true;
		out.eventsCount = eventsData.events?.length;
		out.eventsTotal = eventsData.total;
	} catch (e) {
	...
	// Simulate full getServerSideProps
	try {
		...
		out.fullGsspError = e instanceof Error ? `${e.message}\n${(e as Error).stack?.slice(0, 800)}` : String(e);
	}
